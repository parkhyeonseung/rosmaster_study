...
ImuFilterRos::ImuFilterRos(ros::NodeHandle nh, ros::NodeHandle nh_private) :
    ...
    // **** register publishers
    imu_publisher_ = nh_.advertise<sensor_msgs::Imu>("/pub_imu", 5);

    if (publish_debug_topics_) {
        rpy_filtered_debug_publisher_ = nh_.advertise<geometry_msgs::Vector3Stamped>("/rpy/filtered", 5);

        rpy_raw_debug_publisher_ = nh_.advertise<geometry_msgs::Vector3Stamped>("/rpy/raw", 5);
    }

    // **** register subscribers
    // Synchronize inputs. Topic subscriptions happen on demand in the connection callback.

    imu_subscriber_.reset(new ImuSubscriber(nh_, "/sub_imu", queue_size));

    if (use_mag_) {
        if (use_magnetic_field_msg_) {
            mag_subscriber_.reset(new MagSubscriber(nh_, "/sub_mag", queue_size));
        } else {
            mag_subscriber_.reset(new MagSubscriber(nh_, "/pub_mag", queue_size));
            ...
            mag_republisher_ = nh_.advertise<MagMsg>("/pub_mag", 5);
            vector_mag_subscriber_.reset(new MagVectorSubscriber(nh_, "/sub_mag", queue_size));
    ...
    if (publish_debug_topics_) {
        ...
        if (StatelessOrientation::computeOrientation(world_frame_, lin_acc, mag_compensated, orientation)) {
            tf2::Matrix3x3(tf2::Quaternion(
                    orientation.x, orientation.y, orientation.z, orientation.w)).getRPY(
                            roll, pitch, yaw, 0);
...
void ImuFilterRos::checkTopicsTimerCallback(const ros::TimerEvent &) {
    if (use_mag_)
        ROS_WARN_STREAM("Still waiting for data on topics " << "/sub_mag" << "...");
    else
        ROS_WARN_STREAM("Still waiting for data on topic " << "/sub_imu" << "...");
}