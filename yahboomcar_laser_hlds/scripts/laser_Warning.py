...
class laserWarning:
    def __init__(self):
        ...
        self.ang_pid = SinglePID(0.5, 0.0, .5)
    ...
    def registerScan(self, scan_data):
        ...
        for i in range(len(ranges)):
            ...
            # if (180 + self.laserAngle)>= abs(angle) > (180 - self.laserAngle):
            if 30 >= angle or angle >=330: pass
            else:
                if ranges[i] ==0.:
                    pass
                else:
        ...
        ang_pid_compute = self.ang_pid.pid_compute(180, minDistID)/100.
        if ang_pid_compute > 1: ang_pid_compute = 1.
        print('deg :',minDistID)
        print(ang_pid_compute)
        if 180>minDistID > 0: velocity.angular.z = abs(ang_pid_compute)
        else: velocity.angular.z = -abs(ang_pid_compute)
        if abs(ang_pid_compute) <= 0.05: velocity.angular.z = 0
